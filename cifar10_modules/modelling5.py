...
Original file is located at
    https://colab.research.google.com/drive/1ps3vNXcAYQ66ZY-Ebxz80zVJXluc8Ghy
"""
...
def encoder_conv_blocks(num_filters, en, config):
    ...
    pooling_control_value = config['init_num_filters']*(2)
    regularization_control_value = config['init_num_filters']*(2**(config['conv_blocks']-1))
    dropout_increase = ((math.log(num_filters,2)-4)-1)*0.1
    ...
    for i in range(0,config['layers_per_block']):
        ...
        if config['gaussian_noise_hidden'] and num_filters < regularization_control_value:
            en= GaussianNoise(config['gaussian_noise_stddev'])(en)
    
    if num_filters <= pooling_control_value:
        en= MaxPooling2D((2,2))(en)
    if config['dropout'] and num_filters < regularization_control_value:
        en=Dropout(config['dropout_value']+dropout_increase)(en)
    return en

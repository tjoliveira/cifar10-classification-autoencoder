...
Original file is located at
    https://colab.research.google.com/drive/1gXiDQtmCVwYGTRKIcmY5MUBGzh5l3Nyx
"""
...
from keras.layers import Input, Conv2D, MaxPooling2D, GlobalMaxPooling2D, Reshape, Dense, UpSampling2D, BatchNormalization, Dropout, GaussianNoise
...
def encoder_conv_blocks(num_filters, en, config):
    ...
    for i in range(0,config['layers_per_block']):
        ...
        if config['gaussian_noise_hidden']:
...
def create_encoder(num_filters, en, config):
    ...
    en= GlobalMaxPooling2D()(en)

    en= Dense(num_filters, 
             activation='relu',
...
def create_autoencoder(config):
    ...
    new_shape = config['image_shape'][0] // (2**config['conv_blocks'])
    new_num_filters= num_filters // (new_shape*new_shape)
    print(new_shape)
    print(new_num_filters)

    reshaped_input= Reshape((new_shape, new_shape,new_num_filters))(encoded_input)
    
    de,_= create_decoder(num_filters, reshaped_input,config)
    decoded= Conv2D(3,
                    (3,3), 
                    padding='same', 
                    activation='sigmoid',
